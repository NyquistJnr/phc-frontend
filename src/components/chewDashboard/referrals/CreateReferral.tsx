...
export default function CreateReferral() {
  ...
  const departmentOptions = useMemo(() => {
    ...
    return (departmentsData.results as DepartmentOption[]).map((dept) => ({
      label: dept.name,
      value: dept.id,
    }));
  }, [departmentsData]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      // target_type keeps these mutually exclusive, but guard with `|| null`
      // too so a cleared/never-set field is never sent as an empty string.
      target_doctor_email:
  ...
  return (
    <div className="min-h-screen bg-[#F6F7FC]">
      <ChewDashboardHeader
        title="Referrals"
        breadcrumbs={[
          { label: "Referrals", href: "/chew-dashboard/referrals" },
      ...
      <div className="px-4 py-6 sm:px-6 lg:py-8">
        <ChewBackButton
          onClick={() => router.push("/chew-dashboard/referrals")}
        />
              ...
              <div className="space-y-6">
                <SearchableSelect
                  ...
                  onChange={(value) => {
                    ...
                    if (value !== "SOFTCOPY") {
                      // Hardcopy referrals hand off a physical document —
                      // there's no email target, so clear any prior choice.
                      setForm((current) => ({
                        ...current,
                        target_type: "",
                        target_doctor_email: "",
                ...
                {form.mode_of_referral === "SOFTCOPY" && (
                  <>
                    <div>
