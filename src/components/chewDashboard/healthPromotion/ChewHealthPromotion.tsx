...
import {
  ...
  Pencil, Upload, RefreshCw, Send, Download, Printer
} from "lucide-react";
...
type CampaignStatus = "Scheduled" | "In-Progress" | "Completed" | "Cancelled" | "Draft";
...
const TYPE_OPTIONS        = ["AWARENESS", "VACCINATION_DRIVE", "SCREENING", "EDUCATION"];
const STATUS_OPTIONS      = ["DRAFT", "SCHEDULED", "IN_PROCESS", "CANCELLED", "COMPLETED"];
...
function downloadCSV(filename: string, data: Record<string, any>) {
  ...
  const row = headers.map(header => `"${(data[header] || "").toString().replace(/"/g, '""')}"`).join(",");
...
function FilterDropdown({ label, options, value, onChange }: { label: string; options: string[], value: string, onChange: (v: string) => void }) {
  ...
  const selectedDisplay = value && value !== "All Status" && value !== "All Type" ? value : label;
        ...
        <div ref={menuRef}
          ...
          <div style={{ overflowY: "auto" }} className="py-1.5 px-1.5">
            {[label, ...options].map(opt => {
              const isSelected = value === opt || (opt === label && (!value || value.startsWith("All")));
              return (
                <button key={opt} onClick={() => { onChange(opt === label ? "" : opt); setOpen(false); }}
                  ...
                  <div className="w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors"
                    style={isSelected ? { background: "#046C3F", borderColor: "#046C3F" } : { borderColor: "#D1D5DB" }}>
...
function ActionMenu({ onView, onExportCSV, onPrint }: { onView: () => void; onExportCSV: () => void; onPrint: () => void; }) {
  const [open, setOpen] = useState(false);
  ...
  const items = [
    { label: "View Details", icon: <Eye size={14} />,    action: () => { onView(); setOpen(false); } },
    { label: "Export CSV",   icon: <Download size={14} />, action: () => { onExportCSV(); setOpen(false); } },
    { label: "Print (PDF)",  icon: <Printer size={14} />,  action: () => { onPrint(); setOpen(false); } },
  ];
...
function ExportMenuButton({ onExportCSV, onPrint }: { onExportCSV: () => void; onPrint: () => void; }) {
  const [open, setOpen] = useState(false);
  const [coords, setCoords] = useState({ top: 0, left: 0 });
  const btnRef = useRef<HTMLButtonElement>(null);
  const menuRef = useRef<HTMLDivElement>(null);
...
function FormField({ label, value, onChange, placeholder, disabled, icon, type = "text" }: {
  label: string; value: string; onChange?: (v: string) => void;
  placeholder?: string; disabled?: boolean; icon?: React.ReactNode; type?: string;
}) {
  return (
      ...
      {open && createPortal(
        <div ref={menuRef}
          ...
          <div style={{ overflowY: "auto", flex: 1 }} className="py-1.5 px-1.5">
            ...
            {filtered.length === 0 && <p className="px-4 py-3 text-xs text-gray-400 text-center">No results</p>}
          </div>
        </div>,
        document.body,
      )}
...
function MultiSearchableStaffSelect({ label, value, onChange, disabled }: {
  label: string; value: any[]; onChange: (v: any[]) => void; disabled?: boolean;
}) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [coords, setCoords] = useState({ top: 0, left: 0, maxH: 320, width: 0 });
      ...
      <div ref={trigRef} onClick={toggle}
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-wrap gap-1">
            {value.length === 0 ? (
              <span className={`text-sm text-gray-300 ${disabled ? "text-gray-400" : ""}`}>Select staff...</span>
            ) : (
              value.map(v => (
                <span key={v.id} className="flex items-center gap-1 bg-gray-100 px-2 py-0.5 rounded text-xs text-gray-700">
                  {v.first_name} {v.last_name}
                  {!disabled && <button onClick={(e) => removeSelection(e, v.id)} className="hover:text-red-500"><X size={10} /></button>}
                </span>
              ))
            )}
          </div>
        ...
        <div ref={menuRef}
          ...
          <div style={{ overflowY: "auto", flex: 1 }} className="py-1.5 px-1.5">
            {isLoading ? (
              ...
              return (
                <button key={opt.id} onClick={() => toggleSelection(opt)}
                  className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm text-gray-700 hover:bg-gray-50 transition-colors text-left">
                  <div className="w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors"
                    style={isSelected ? { background: "#046C3F", borderColor: "#046C3F" } : { borderColor: "#D1D5DB" }}>
                    {isSelected && <Check size={10} color="#fff" strokeWidth={3} />}
            ...
            {!isLoading && (!staffList || staffList.length === 0) && <p className="px-4 py-3 text-xs text-gray-400 text-center">No results</p>}
          </div>
        </div>,
        document.body,
      )}
...
function SearchableHealthPromotionSelect({ label, value, onChange, disabled }: {
  label: string; value: string; onChange: (id: string, title: string) => void; disabled?: boolean;
}) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [coords, setCoords] = useState({ top: 0, left: 0, maxH: 320, width: 0 });
  ...
  const { data: promotionsData, isLoading } = useHealthPromotions({ search: query, page: 1, page_size: 10 });
  ...
  const selectedDisplay = value || "Select Health Promotion...";
        ...
        <div ref={menuRef}
          ...
          <div style={{ overflowY: "auto", flex: 1 }} className="py-1.5 px-1.5">
            {isLoading ? (
              ...
              return (
                <button key={opt.id} onClick={() => { onChange(opt.id, opt.title); setOpen(false); }}
                  ...
                  <span className="text-xs text-gray-400 ml-6">{opt.id?.substring(0,8)} • {opt.type}</span>
                </button>
              );
            })}
            {!isLoading && promotions.length === 0 && <p className="px-4 py-3 text-xs text-gray-400 text-center">No results</p>}
          </div>
        </div>,
        document.body,
      )}
...
function HealthPromotionList({
  onNewActivity, onNewPost, onViewPost, onViewActivity,
}: {
  onNewActivity: () => void;
  onNewPost: () => void;
  ...
  const { data: hpData, isLoading: hpLoading } = useHealthPromotions({ page, page_size: 10, search, type, status });
  ...
  const { data: paData, isLoading: paLoading } = usePostActivities({ page: postPage, page_size: 10, search: postSearch, type: postType, status: postStatus });

  return (
    <div className="flex-1 flex flex-col bg-[#F9FAFB] min-w-0">
      <ChewDashboardHeader
        title="Health Promotion"
        breadcrumbs={[{ label: "Chew Dashboard" }, { label: "Health Promotion", active: true }]}
      />
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10 space-y-5">
        ...
        {activeTab === "health" && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="relative">
                  ...
                  <input placeholder="Search by Activity title or ID..." value={search} onChange={(e) => setSearch(e.target.value)}
                ...
                <FilterDropdown label="All Type"   options={TYPE_OPTIONS} value={type} onChange={setType} />
                <FilterDropdown label="All Status" options={STATUS_OPTIONS} value={status} onChange={setStatus} />
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[860px]">
                ...
                <tbody className="divide-y divide-gray-50">
                  {hpLoading ? (
                    <tr><td colSpan={8} className="text-center py-6 text-sm text-gray-500">Loading...</td></tr>
                  ) : hpData?.results?.length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-6 text-sm text-gray-500">No records found.</td></tr>
                  ) : (
                    hpData?.results?.map((row, i) => (
                      <tr key={i} className="hover:bg-gray-50/60 transition-colors">
                        <td className="px-5 py-3 text-sm text-gray-600 font-medium">{row.id?.substring(0, 8)}</td>
                        <td className="px-5 py-3 text-sm text-gray-700">{row.title}</td>
                        <td className="px-5 py-3 text-sm text-gray-500">{row.type}</td>
                        <td className="px-5 py-3 text-sm text-gray-500">{row.expected_participants}</td>
                        <td className="px-5 py-3 text-sm text-gray-500 whitespace-nowrap">
                          {formatDateStr(row.start_date)} - {formatDateStr(row.end_date)}
                        </td>
                        <td className="px-5 py-3 text-sm text-gray-500">{row.created_by}</td>
                        <td className="px-5 py-3">
                          <span className="px-2.5 py-1 rounded-lg text-xs font-semibold whitespace-nowrap" style={STATUS_STYLE[row.status] || STATUS_STYLE.DRAFT}>
                            {row.status}
                          </span>
                        </td>
                        <td className="px-5 py-3">
                          <ActionMenu
                            ...
                            onExportCSV={() => downloadCSV(`campaign_${row.id}.csv`, row)}
                            onPrint={() => printDetails("Health Promotion details", row)}
                          />
                        </td>
                      </tr>
                    ))
            ...
            {hpData && <Pagination currentPage={page} totalPages={hpData.total_pages} onPageChange={setPage} />}
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="relative">
                  <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                  <input placeholder="Search by Activity title or ID..." value={postSearch} onChange={(e) => setPostSearch(e.target.value)}
                    className="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-[#046C3F] w-64" />
                </div>
                <PeriodFilterButton label="Date Range" />
                <FilterDropdown label="All Type"   options={TYPE_OPTIONS} value={postType} onChange={setPostType} />
                <FilterDropdown label="All Status" options={POST_STATUS_OPTIONS} value={postStatus} onChange={setPostStatus} />
              </div>
            </div>
            <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-gray-50">
                  {paLoading ? (
                    ...
                    paData?.results?.map((row, i) => (
                      <tr key={i} className="hover:bg-gray-50/60 transition-colors">
                        <td className="px-5 py-3 text-sm text-gray-600 font-medium">{row.id?.substring(0, 8)}</td>
                        <td className="px-5 py-3 text-sm text-gray-700">{row.promotion_title || "N/A"}</td>
                        <td className="px-5 py-3 text-sm text-gray-500">{row.promotion_type || "N/A"}</td>
                        <td className="px-5 py-3 text-sm text-gray-500">{row.number_of_participants}</td>
                        <td className="px-5 py-3 text-sm text-gray-500 whitespace-nowrap">
                          {formatDateStr(row.created_at)}
                        </td>
                        <td className="px-5 py-3 text-sm text-gray-500">{row.outcome_summary}</td>
                        <td className="px-5 py-3">
                          <span className="px-2.5 py-1 rounded-lg text-xs font-semibold whitespace-nowrap" style={POST_STATUS_STYLE[row.status] || POST_STATUS_STYLE.DRAFT}>
                            {row.status}
                          </span>
                        </td>
                        <td className="px-5 py-3">
                          <ActionMenu
                            ...
                            onExportCSV={() => downloadCSV(`post_activity_${row.id}.csv`, row)}
                            onPrint={() => printDetails("Post Activity details", row)}
                          />
                        </td>
                      </tr>
                    ))
            ...
            {paData && <Pagination currentPage={postPage} totalPages={paData.total_pages} onPageChange={setPostPage} />}
          </div>
        )}
      </div>
    </div>
...
function HealthActivityForm({
  ...
  const [participants, setParticipants]   = useState(initialData?.expected_participants || 0);
  ...
  const [assignTo, setAssignTo]           = useState<any[]>(
    initialData?.assigned_to?.map(id => ({ id, first_name: "Staff", last_name: "Member" })) || []
  );
    ...
    <div className="flex-1 flex flex-col bg-[#F9FAFB] min-w-0">
      <ChewDashboardHeader
        ...
        breadcrumbs={[
          ...
          { label: currentMode === "new" ? "New Activity" : currentMode === "edit" ? "Edit Activity" : "View Activity", active: true },
        ]}
      />
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10 space-y-5">

        <button onClick={onBack} className="flex items-center gap-1.5 text-sm font-semibold text-[#046C3F] hover:opacity-80 transition-opacity">
          <ArrowLeft size={16} /> Back
        </button>

        <div className="flex items-center justify-between gap-4">
          <h1 className="text-2xl font-bold text-gray-900">
            {currentMode === "new" ? "Create a Health promotion activity" : currentMode === "edit" ? "Edit Activity" : "View Activity"}
          </h1>
          {currentMode === "view" && initialData && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              <ExportMenuButton
                onExportCSV={() => downloadCSV(`campaign_${initialData.id}.csv`, initialData)}
                onPrint={() => printDetails("Health Promotion details", initialData)}
              />
            </div>
          )}
        </div>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 cursor-pointer"
            ...
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-bold text-gray-800">
                {currentMode === "new" ? "Create a new activity" : currentMode === "edit" ? "Edit activity" : "View activity"}
              </h2>
            </div>
          ...
          {sectionOpen && (
            <div className="px-6 py-6 space-y-4">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <FormField label="Activity title" value={activityTitle} onChange={currentMode === "view" ? undefined : setActivityTitle}
                  placeholder="e.g Malaria prevention awareness" disabled={currentMode === "view"} />
                <FormField label="Campaign ID" value={initialData?.id || ""} placeholder="Auto-generated" disabled />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <FormSelect label="Type" value={type} onChange={setType} options={TYPE_OPTIONS} disabled={currentMode === "view"} />
                <FormField label="Location" value={location} onChange={currentMode === "view" ? undefined : setLocation}
                  placeholder="e.g Gwagwalada market" disabled={currentMode === "view"} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <FormField label="Target audience" value={targetAudience} onChange={currentMode === "view" ? undefined : setTargetAudience}
                  placeholder="e.g Pregnant women" disabled={currentMode === "view"} />
                <NumberStepper label="Expected Participants" value={participants} onChange={setParticipants} disabled={currentMode === "view"} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <FormField label="Start Date" value={startDate} onChange={currentMode === "view" ? undefined : setStartDate} type="date"
                   disabled={currentMode === "view"} />
                <FormField label="End Date" value={endDate} onChange={currentMode === "view" ? undefined : setEndDate} type="date"
                   disabled={currentMode === "view"} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                {currentMode !== "new" && (
                  <SearchableFormSelect label="Created by" value={initialData?.created_by || ""} onChange={() => {}} options={[]} disabled />
                )}
                <div style={{ gridColumn: currentMode === "new" ? "1 / span 2" : "auto" }}>
                  <MultiSearchableStaffSelect label="Assign to" value={assignTo} onChange={setAssignTo} disabled={currentMode === "view"} />
                </div>
              </div>
              <FormTextarea label="Description" value={description} onChange={currentMode === "view" ? undefined : setDescription}
                placeholder="Briefly describe the activity and key outcome..." disabled={currentMode === "view"} />
              ...
              {currentMode !== "view" && (
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <button onClick={() => handleSubmit("SCHEDULED")} disabled={isSaving}
                    className="px-8 py-2.5 rounded-xl text-sm font-semibold text-white transition-colors flex items-center gap-2"
                    style={{ background: "#046C3F" }}>
                    {isSaving ? "Saving..." : currentMode === "new" ? "Submit Activity" : <><RefreshCw size={15} /> Update Changes</>}
                  </button>
                </div>
              )}
            </div>
...
function PostActivityForm({
  ...
  const [healthPromotion, setHealthPromotion] = useState(initialData?.health_promotion || "");
  const [activityTitle, setActivityTitle] = useState(initialData?.promotion_title || "");

  // Participation fields
  const [numParticipants, setNumParticipants] = useState(initialData?.number_of_participants || 0);
  const [maleCount, setMaleCount]             = useState(initialData?.male_count || 0);
  const [femaleCount, setFemaleCount]         = useState(initialData?.female_count || 0);
  const [followUp, setFollowUp]               = useState(initialData?.follow_up_required || false);
  const [keyMessages, setKeyMessages]         = useState(initialData?.key_messages_delivered || "");
  const [outcomeSummary, setOutcomeSummary]   = useState(initialData?.outcome_summary || "");
    ...
    <div className="flex-1 flex flex-col bg-[#F9FAFB] min-w-0">
      <ChewDashboardHeader
        ...
        breadcrumbs={[
          { label: "Chew Dashboard" },
          { label: "Health Promotion" },
          { label: currentMode === "new" ? "New Post Activity" : currentMode === "edit" ? "Edit Post Activity" : "View Post Activity", active: true },
        ]}
      />
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10 space-y-5">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {currentMode === "view" && initialData && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              <ExportMenuButton
                onExportCSV={() => downloadCSV(`post_activity_${initialData.id}.csv`, initialData)}
                onPrint={() => printDetails("Post Activity details", initialData)}
              />
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 cursor-pointer"
            ...
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-bold text-gray-800">{currentMode === "view" ? "View post activity" : "Record Post Activity"}</h2>
            </div>
            <button className="text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <div className="px-6 py-6 space-y-4">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <SearchableHealthPromotionSelect
                  ...
                  onChange={(id, title) => { setHealthPromotion(id); setActivityTitle(title); }}
                  disabled={currentMode === "view"}
                />
                <FormField label="Activity Title" value={activityTitle} onChange={() => {}}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          {section2Open && (
            <div className="px-6 py-6 space-y-4">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <NumberStepper label="Number of Participants" value={numParticipants} onChange={setNumParticipants} disabled={currentMode === "view"} />
                <NumberStepper label="Male Count" value={maleCount} onChange={setMaleCount} disabled={currentMode === "view"} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <NumberStepper label="Female Count" value={femaleCount} onChange={setFemaleCount} disabled={currentMode === "view"} />
                <div className="flex flex-col gap-1.5 px-1">
                  <button className="flex items-center gap-2" onClick={() => currentMode !== "view" && setFollowUp(v => !v)} disabled={currentMode === "view"}>
                    <div className="w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors"
              ...
              <FormTextarea label="Key Messages Delivered" value={keyMessages} onChange={currentMode === "view" ? undefined : setKeyMessages}
                placeholder="e.g. Hygiene, Vaccination awareness" disabled={currentMode === "view"} />
              <FormTextarea label="Outcome Summary" value={outcomeSummary} onChange={currentMode === "view" ? undefined : setOutcomeSummary}
                placeholder="e.g. Increased awareness" disabled={currentMode === "view"} />
              <FormTextarea label="Challenges" value={challenges} onChange={currentMode === "view" ? undefined : setChallenges}
                placeholder="Challenges Faced" disabled={currentMode === "view"} />
              ...
              {currentMode !== "view" && (
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <button onClick={() => handleSubmit(currentMode === "new" ? "SUBMITTED" : status)} disabled={isSaving}
                    className="flex items-center gap-2 px-8 py-2.5 rounded-xl text-sm font-semibold text-white transition-colors"
                    style={{ background: "#046C3F" }}>
                    {isSaving ? "Saving..." : currentMode === "new" ? <><Send size={15} /> Submit</> : <><RefreshCw size={15} /> Update Changes</>}
                  </button>
                </div>
              )}
            </div>
...
type ViewState = "list" | "new-activity" | "new-post" | "view-post" | "view-activity";
...
export default function ChewHealthPromotion() {
  ...
  const [selectedActivity, setSelectedActivity] = useState<HealthPromotion | null>(null);
  const [selectedPost, setSelectedPost] = useState<PostActivityReport | null>(null);

  if (view === "new-activity") return <HealthActivityForm mode="new" onBack={() => setView("list")} />;
  if (view === "new-post")     return <PostActivityForm mode="new"  onBack={() => setView("list")} />;
  if (view === "view-post" && selectedPost)    return <PostActivityForm mode="view" initialData={selectedPost} onBack={() => setView("list")} />;
  if (view === "view-activity" && selectedActivity) return (
    <HealthActivityForm mode="view" initialData={selectedActivity} onBack={() => setView("list")} />
  );
  ...
  return (
    <HealthPromotionList
      ...
      onViewPost={(row) => { setSelectedPost(row); setView("view-post"); }}
      onViewActivity={(row) => { setSelectedActivity(row); setView("view-activity"); }}
    />
  );
}
