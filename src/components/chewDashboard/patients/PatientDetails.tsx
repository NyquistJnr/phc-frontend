    ...
    <GenericTable
      ...
      columns={[
        ...
        { header: "Notes", accessorKey: "notes", sortable: true },
        {
          ...
          render: (row) => (
            <Link
              href={`/chew-dashboard/appointments/${row.rawId}`}
  ...
  const items = [
    {
      label: "View Detail",
      icon: Eye,
      onClick: () => router.push(`/chew-dashboard/referrals/${row.id}`),
  ...
  return (
    <>
      <ChewDashboardHeader
        title="Patients"
        breadcrumbs={[
          { label: "Patients", href: "/chew-dashboard/patients" },
          { label: "View Profile" },
        ]}
      />
      <div className="px-4 py-6 sm:px-6 lg:py-8">
        <ChewBackButton onClick={onBack} />
