...
import PostnatalCareTab from "./tabs/PostnatalCareTab";
import MaternalCareTab from "./tabs/MaternalCareTab";
import FollowUpsTab from "./tabs/FollowUpsTab";
import HealthEducationTab from "./tabs/HealthEducationTab";
...
const TABS = [
  { id: "vital-signs", label: "Vital Signs", icon: Activity, Component: VitalSignsTab },
  { id: "immunization", label: "Immunization", icon: Syringe, Component: ImmunizationTab },
  { id: "maternal-care", label: "Maternal Care (ANC)", icon: Heart, Component: MaternalCareTab },
  { id: "postnatal-care", label: "Postnatal Care (PNC)", icon: Baby, Component: PostnatalCareTab },
  { id: "follow-ups", label: "Follow-ups", icon: ListTodo, Component: FollowUpsTab },
  { id: "health-education", label: "Health Education", icon: BookOpen, Component: HealthEducationTab },
] as const;
...
export default function NurseReports() {
  ...
  // Follow-ups report is a live worklist and ignores start/end date for active items.
  const isLiveWorklist = activeTab === "follow-ups";

  return (
    <div className="flex-1 flex flex-col bg-[#F8FAFC] min-w-0 overflow-hidden">
      <NurseDashboardHeader title="Reports" breadcrumbs={breadcrumbs} />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-white p-6 sm:p-8 rounded-[20px] shadow-sm border border-gray-100 print:hidden">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-bold text-[#046C3F] uppercase tracking-wider">My Facility Reports</p>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">{active.label}</h1>
            <p className="text-gray-500 mt-2 text-sm">
              {isLiveWorklist ? "Live task list (date filter applies to completed items only)" : getPeriodLabel(range.start, range.end)}
            </p>
          </div>
