        ...
        <div className="flex items-center gap-4 sm:gap-6">
          <div className="relative hidden md:block w-64 lg:w-80 xl:w-[450px]">
            ...
            <input
              type="text"
              placeholder="Search"
              className="w-full bg-white border border-gray-200 rounded-full pl-12 pr-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-[#046C3F]"
            />
          </div>

          <div className="flex items-center gap-3 sm:gap-4">
            <button
              onClick={() => router.push("/iho-dashboard/notifications")}
              className="relative p-2 bg-gray-50 rounded-full text-gray-600 hover:bg-gray-100 transition-colors"
            >
              <Bell size={20} />
              {unreadCount > 0 && (
                <span className="absolute top-1.5 right-1.5 min-w-[16px] h-4 px-1 bg-[#046C3F] text-white text-[9px] font-bold rounded-full border-2 border-white flex items-center justify-center">
      ...
      <nav className="bg-[#F9FAFB] px-2 sm:px-8 py-3 sm:py-4 flex items-center gap-2 text-sm text-gray-500 overflow-x-auto">
        ...
        {breadcrumbs.map((crumb, index) => (
          <React.Fragment key={index}>
            ...
            <span
              className={`whitespace-nowrap ${crumb.href ? "text-gray-700" : "text-gray-800 font-medium"}`}
            >
              {crumb.href ? (
