...
export default function Dashboard() {
  ...
  return (
      <div className="flex-1 flex flex-col bg-[#F9FAFB]">
        ...
        <div className="flex-1 px-4 sm:px-8 pt-4 pb-10 space-y-8">
          
          {/* Welcome Section */}
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
              <h2 className="text-3xl font-medium font-inter text-gray-900 tracking-tight">Welcome Nobert</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-100">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="text-[10px] text-gray-400 font-bold flex items-center gap-1 cursor-pointer">
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50">
                    <th className="px-8 py-5"><div className="flex items-center gap-2">User <ListFilter size={14} /></div></th>
                    <th className="px-6 py-5"><div className="flex items-center gap-2">Action <ListFilter size={14} /></div></th>
                    <th className="px-6 py-5"><div className="flex items-center gap-2">Module <ListFilter size={14} /></div></th>
                    <th className="px-6 py-5"><div className="flex items-center gap-2">Timestamp <ListFilter size={14} /></div></th>
                    <th className="px-6 py-5"><div className="flex items-center gap-2">IP Address <ListFilter size={14} /></div></th>
                    <th className="px-6 py-5 text-center"><div className="flex items-center justify-center gap-2">Status <ListFilter size={14} /></div></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {auditLogs.map((log, index) => (
                    <tr key={index} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-8 py-5 text-sm font-semibold text-gray-700">{log.user}</td>
                      <td className="px-6 py-5 text-sm text-gray-500">{log.action}</td>
                      <td className="px-6 py-5 text-sm text-gray-500">{log.module}</td>
                      <td className="px-6 py-5 text-sm text-gray-500 font-mono">{log.timestamp}</td>
                      <td className="px-6 py-5 text-sm text-gray-500 font-mono">{log.ip}</td>
                      <td className="px-6 py-5">
                        <div className="flex justify-center">
                          <span className={`${log.statusColor} px-3 py-1 rounded-full text-[10px] font-bold`}>{log.status}</span>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-100">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-xs text-gray-400 font-medium">Security and operational warnings</p>
              </div>
              <button className="flex items-center gap-2 text-[#046C3F] text-sm font-bold">
                View all <ArrowRight size={16} />
              </button>
            </div>
