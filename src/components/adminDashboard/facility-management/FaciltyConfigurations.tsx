...
// Added onClick to the props so the select field can be toggled
const LabeledInput = ({ label, onClick, ...props }: { label: string; onClick?: () => void; [key: string]: unknown }) => {
  ...
  return (
    <div className="relative">
      ...
      {props.type === 'select' ? (
        <div onClick={onClick} className={`${inputStyles} flex items-center cursor-pointer`}>
          <span className="flex-grow">{String(props.value ?? '')}</span>
...
export default function FacilityConfigurationPage() {
  ...
  // States for the Dropdown functionality
  const [idLength, setIdLength] = useState('6 digits');
  ...
  // Helper to dynamically calculate the preview based on selection
  const numDigits = parseInt(idLength.split(' ')[0]);
  const dynamicPreview = `PAT-PLT-${"234".padStart(numDigits, '0')}`;
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 p-4 sm:p-8 space-y-6 sm:space-y-8 max-w-5xl">
        ...
        <div className="bg-white p-4 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <LabeledInput label="Prefix" value="PAT-PLT" readOnly />
            ...
            <div className="relative">
              ...
              {isDropdownOpen && (
                <div className="absolute top-[3.5rem] left-0 w-full bg-white border border-gray-100 rounded-2xl shadow-xl z-20 py-2 animate-in fade-in slide-in-from-top-2 duration-200">
                  {['6 digits', '7 digits', '8 digits', '9 digits', '10 digits'].map((opt) => {
                    const isSelected = opt === idLength;
                    return (
                      <div 
                        key={opt} 
                        onClick={() => {
                      ...
                      >
                        <div className={`w-5 h-5 rounded-md border flex items-center justify-center ${isSelected ? 'bg-[#1AC073] border-[#1AC073]' : 'border-gray-300'}`}>
            ...
            <div className="md:col-span-2">
              ...
              <p className="text-lg font-bold text-[#1AC073]">{dynamicPreview}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 sm:p-8 rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 space-y-6 sm:space-y-8">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 pt-6">
            <button onClick={handleSave} className="px-8 sm:px-10 py-3.5 bg-[#046C3F] text-white rounded-xl font-semibold flex items-center gap-2.5 shadow-md hover:bg-[#035a34] transition">
              <Settings2 size={20} />
              Save Configuration
            </button>
      ...
      <Toast
        ...
        message="Facility settings have been updated successfully."
