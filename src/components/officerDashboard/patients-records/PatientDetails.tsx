...
function DemographicsTab({ patientId }: { patientId: string }) {
  ...
  const phoneValue = patient.phone_number ? (
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white px-5 py-7 shadow-sm sm:px-6 lg:px-8">
      <div className="mb-8 flex items-center gap-3">
        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#EBF7F2] text-[#2A6543]">
          <User size={18} />
        </span>
      ...
      <div className="max-w-4xl space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          />
          <Field label="Phone Number" value={phoneValue} muted />
        </div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div className="rounded-lg border border-gray-300 bg-white px-4 py-3">
          ...
          <div className="flex items-center gap-5 px-8">
            ...
            >
              <span
                className={`absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full text-xs text-white ${patient.is_active ? "bg-[#2A6543]" : "bg-gray-500"}`}
              >
                {patient.is_active ? "✓" : "x"}
              </span>
            </span>
    ...
    <GenericTable
      ...
      columns={[
        ...
        { header: "Notes", accessorKey: "notes", sortable: true },
        {
          ...
          render: (row) => (
            <button
              onClick={() => alert(`View history ${row.id}`)}
              className="text-[#2A6543] font-medium hover:underline"
            >
              View
            </button>
          ),
    ...
    <GenericTable
      ...
      columns={[
        ...
        },
        {
          ...
          render: (row) => (
            <button
              onClick={() => alert(`View medication ${row.id}`)}
              className="text-[#2A6543] font-medium hover:underline"
            >
              View
            </button>
...
function ReferralsTab({ patientId }: { patientId: string }) {
  ...
  const columns: ColumnDef<PatientReferral>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <button
          onClick={() => alert(`View referral ${row.referral_id}`)}
          className="text-[#2A6543] font-medium hover:underline"
        >
          View
        </button>
  ...
  return (
    <>
      <OfficerDashboardHeader
        title="Patients"
        breadcrumbs={[
          { label: "Patient Records", href: "/oic-dashboard/patient-records" },
          { label: "View Profile" },
        ]}
      />
      <div className="px-4 py-6 sm:px-6 lg:py-8">
        <OfficerBackButton onClick={onBack} />
