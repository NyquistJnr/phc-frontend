...
import { VitalsForm } from "../../nurse-dashboard/vitals/type";
...
export default function RecordVitalDoctor() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    createVital(payload, {
      onSuccess: () => {
        ...
        setTimeout(() => {
          router.push(`/doctor-dashboard/appointments/${appointmentId}`);
        }, 1500);
      },
      onError: (error: any) => {
        const errorMsg =
  ...
  return (
    <div className="min-h-screen bg-[#F6F7FC]">
      <DoctorHeader title="Vitals" breadcrumbs={[{ label: "Vitals" }]} />
