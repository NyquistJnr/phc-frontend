  ...
  return (
    <div className="fixed bottom-8 right-8 z-50 flex w-[min(390px,calc(100vw-2rem))] items-center gap-4 rounded border border-gray-200 bg-white px-5 py-3 shadow-sm">
      ...
      <div className="flex-1">
        <p className="text-sm font-semibold text-gray-900">Patient Updated</p>
...
export default function EditPatient() {
  ...
  // Fetch Existing Data & Setup Mutations
  ...
  useEffect(() => {
    if (patientData) {
      setForm({
        ...
        // Map the nested profile properties onto the flat payload structure
        sex: patientData.profile?.sex || "",
        date_of_birth: patientData.profile?.date_of_birth || "",
        lga: patientData.profile?.lga || "",
        ward: patientData.profile?.ward || "",
        next_of_kin_name: patientData.profile?.next_of_kin_name || "",
        next_of_kin_phone: patientData.profile?.next_of_kin_phone || "",
        insurance_status: patientData.profile?.insurance_status || "",
        insurance_provider: patientData.profile?.insurance_provider || "",
        insurance_package: patientData.profile?.insurance_package || "",
        coverage_status: patientData.profile?.coverage_status || "",
        allergies: patientData.profile?.allergies || "",
        chronic_conditions: patientData.profile?.chronic_conditions || "",
        notes: patientData.profile?.notes || "",
        blood_group: patientData.profile?.blood_group || "",
        genotype: patientData.profile?.genotype || "",
      });
    }
  }, [patientData]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    updatePatient(
      { id: patientId, payload: form },
      {
        onSuccess: () => {
          setToastVisible(true);
          setTimeout(() => {
            router.push(`/nurse-dashboard/patients/${patientId}`);
          }, 2000);
        },
        onError: (error: any) => {
          setFormError(
  ...
  return (
    <div className="min-h-screen bg-[#F6F7FC]">
      <NurseDashboardHeader
        title="Patients"
        breadcrumbs={[{ label: "Patients" }, { label: "Edit Patient" }]}
      />
