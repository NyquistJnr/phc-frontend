  ...
  return useMemo(
    () => ({
      get: <T>(endpoint: string, options?: any) =>
        authenticatedFetch<T>("get", endpoint, undefined, options),
      post: <T>(endpoint: string, body?: any, options?: any) =>
        authenticatedFetch<T>("post", endpoint, body, options),
      put: <T>(endpoint: string, body?: any, options?: any) =>
        authenticatedFetch<T>("put", endpoint, body, options),
      patch: <T>(endpoint: string, body?: any, options?: any) =>
        authenticatedFetch<T>("patch", endpoint, body, options),
      delete: <T>(endpoint: string, options?: any) =>
        authenticatedFetch<T>("delete", endpoint, undefined, options),
